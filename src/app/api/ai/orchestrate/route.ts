...
export async function POST(req: NextRequest) {
  try {
    ...
    // Safely extract session ID snippet
    const rawCookie = req.cookies.get("bms_session")?.value;
    const sessionIdSnippet = rawCookie ? rawCookie.slice(0, 8) + "…" : "unknown";
    ...
    console.error("Orchestration API Error Stack:", error?.stack || error);
    return NextResponse.json(
      {
