-- ============================================================
-- Migration: 0015_bms_ops_intelligence.sql
-- Purpose:  BMS Ops Intelligence: Code Red, Assignments,
--           Documents, Gates
...
-- ── 5. Missing Nidhivan RLS policies (remediation) ──────────
-- Resolves nidhivan_boqs, nidhivan_boq_items, nidhivan_financial_metrics
-- returning empty arrays (only nidhivan_dprs had a policy from 0012).

CREATE POLICY IF NOT EXISTS nidhivan_boqs_tenant_isolation ON nidhivan_boqs
  FOR ALL TO studio_app
  USING  (tenant_id = current_setting('app.current_tenant_id', true)::integer)
  WITH CHECK (tenant_id = current_setting('app.current_tenant_id', true)::integer);

CREATE POLICY IF NOT EXISTS nidhivan_boq_items_tenant_isolation ON nidhivan_boq_items
  FOR ALL TO studio_app
  USING  (tenant_id = current_setting('app.current_tenant_id', true)::integer)
  WITH CHECK (tenant_id = current_setting('app.current_tenant_id', true)::integer);

CREATE POLICY IF NOT EXISTS nidhivan_financial_metrics_tenant_isolation ON nidhivan_financial_metrics
  FOR ALL TO studio_app
  USING  (tenant_id = current_setting('app.current_tenant_id', true)::integer)
  WITH CHECK (tenant_id = current_setting('app.current_tenant_id', true)::integer);

CREATE POLICY IF NOT EXISTS nidhivan_projects_tenant_isolation ON nidhivan_projects
  FOR ALL TO studio_app
  USING  (tenant_id = current_setting('app.current_tenant_id', true)::integer)
  WITH CHECK (tenant_id = current_setting('app.current_tenant_id', true)::integer);
